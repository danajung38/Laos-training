    ...
    weeks: [
      {
        ...
        sessions: [
          ...
          },
          {
            ...
            guest: 'Mr. Bounmy Phommasack (LSEA)',
...
const SESSION_TYPE_CONFIG = {
  lecture:   { label: '강의',    color: '#ffffff', bg: '#1a3a6b' },  /* 진한 네이비 + 흰 글씨 */
  special:   { label: '특강',    color: '#ffffff', bg: '#7c3aed' },  /* 보라 + 흰 글씨 */
  fieldtrip: { label: '필드트립', color: '#ffffff', bg: '#16803c' },  /* 초록 + 흰 글씨 */
  culture:   { label: '문화체험', color: '#ffffff', bg: '#ff8e60' },  /* 오렌지 + 흰 글씨 */
};
...
(function () {
  ...
  function buildSessionHTML(session) {
    ...
    var badge =
      ...
      '</span>';
    var sub = '';
    if (session.guest)    sub = '<span class="curr-session-sub">👤 ' + session.guest + '</span>';
  ...
  function renderList() {
    ...
    monthData.weeks.forEach(function (item) {
      ...
      var sessionsHTML = (item.sessions || []).map(buildSessionHTML).join('');
      ...
      li.innerHTML =
        ...
        '<div class="curr-card-detail">' +
          '<div class="curr-card-detail-inner">' +
            '<p class="curr-professor">🎓 ' + item.professor + '</p>' +
